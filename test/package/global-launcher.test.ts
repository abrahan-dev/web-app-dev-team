...
describe("global launcher", () => {
  ...
  test("requires an explicit run workspace", () => {
    const result = Bun.spawnSync(["node", launcher, "run", "--prompt", "Build an application"], {
      cwd: "/tmp",
      env: { ...process.env, WEB_APP_DEV_TEAM_BUN: Bun.which("bun") ?? "bun" },
      stderr: "pipe",
      stdout: "pipe",
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr.toString()).toContain("Missing required argument --workspace");
  });
});
